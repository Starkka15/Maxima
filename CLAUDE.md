...
- **D6 — run-game-once enforcement for Steam/EA paths (Draconis-side).** When a user picks the Steam or EA route in Draconis's Onboarding wizard, the in-bottle launcher needs the game to be run at least once *through that launcher* before Draconis can take over — Steam to populate its CEG-signing / shader cache, EA Desktop to finish its first-run profile setup. The current wizard copy ([OnboardingView.swift](https://github.com/AA-EION/Draconis/blob/main/Draconis/Views/OnboardingView.swift) `stepTwoDetail`) *tells* the user to do this, but `nextPageForExistingBottle` advances on `hasTitanfall2 && installComplete` without checking any first-run artifact, so a user who skips the manual launch step still gets dismissed into PlayView and then hits an obscure failure on first Draconis launch. Deferred until we have empirical signal on what each launcher writes after a successful first run on Wine — guessing wrong here would block the wizard for users who legitimately *did* run the game once. Candidate signals: Steam writes to `userdata/<steamid>/1237970/`; EA Desktop touches `%USERPROFILE%/Documents/Respawn/Titanfall2/local/profile.cfg`; both untested in CrossOver. Until then, **users on Steam/EA paths who forget the manual run-once step will hit a generic launch error rather than a clear wizard-level message**.
